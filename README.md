...
As a conservative estimate, the Delique operating system should be in a state where it is fully functional and integrated with the NsCDE desktop environment by January-February 2026.
...
The deviations from the Linux From Scratch books so far include:
  ...
  - systemd and the dbus-daemon are not installed correctly due to a bug potentially not specified in the Linux From Scratch book, with utilities from systemd affected by it intentionally disabled and the dbus-daemon being launched manually for now without integration with systemd.
